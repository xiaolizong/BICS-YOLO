...
class HUBDatasetStats:
    ...
    Args:
        path (str): Path to streeview.yaml or data.zip (with streeview.yaml inside data.zip). Default is 'coco8.yaml'.
    ...
    def _unzip(path):
        """Unzip data.zip."""
        if not str(path).endswith('.zip'):  # path is streeview.yaml
